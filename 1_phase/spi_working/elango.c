...
static void elango_prepare_spi_message(void)
{
	...
	elango_ctl.tx_buff[0] = elango_dev.test_data++;
	elango_ctl.tx_buff[1] = elango_dev.test_data++;
			...
			loff_t *offp)
{
	...
	if (status) {
		...
		sprintf(elango_dev.user_buff, 
			"Status: %d\nTX: %d %d %d %d\nRX: %d %d %d %d\n",
			elango_ctl.msg.status,
			elango_ctl.tx_buff[0], elango_ctl.tx_buff[1], 
			elango_ctl.tx_buff[2], elango_ctl.tx_buff[3],
			elango_ctl.rx_buff[0], elango_ctl.rx_buff[1], 
			elango_ctl.rx_buff[2], elango_ctl.rx_buff[3]);
	}
